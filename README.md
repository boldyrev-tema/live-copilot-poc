...
- **Cerebras как возможная замена Groq для основной LLM — не проверено,
  только исследовано.** Бесплатный тир: 1М токенов/день, 30 запросов/мин,
  без карты, заявленная скорость 2600+ токенов/сек на Llama 4 Scout —
  на бумаге быстрее Groq. НЕ протестировано реальным вызовом (нет
  аккаунта/ключа) — throughput в токенах/сек не то же самое, что задержка
  на короткий ответ (3-5 предложений), разница может не проявиться на
  практике. Если понадобится — завести ключ и замерить на тех же тестовых
  вопросах, что в `test_poc.py`, прежде чем менять `LLM_MODEL`.
