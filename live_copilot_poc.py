...
# Минимум символов в репликах "Ты" для отчёта по сессии — живой тест поймал
# реальную галлюцинацию модели на слишком коротком транскрипте (см. generate_session_report).
MIN_REPORT_CANDIDATE_CHARS = 60
...
def strip_math_markup(text: str) -> str:
    ...
    # Живой тест на скриншоте с "8^2" показал: модель пишет степень через "^"
    # вопреки промпту, и это раньше проходило через фильтр буквально.
    text = re.sub(r"(\w+)\s*\^\s*(\w+)", r"\1 в степени \2", text)
    text = text.replace("^", "")
    # То же для нижних индексов через "_" (упомянуты в промпте как запрещённые,
    # но не было отдельной проверки, что модель их правда так пишет).
    text = re.sub(r"(\w+)\s*_\s*(\w+)", r"\1 с индексом \2", text)
    return text
    ...
    def worker():
        try:
            if not transcript_lines:
                ...
                return
            # Живой тест поймал реальную галлюцинацию: на транскрипте из одного
            # "Собеседник: Привет" модель написала отчёт с ПОЛНОСТЬЮ выдуманными
            # цитатами кандидата ("я разработал модуль аналитики...") — их не
            # было в транскрипте вообще. Минимум объёма реплик "Ты" — защита от
            # этого, не просто "мало текста для интересного отчёта".
            candidate_chars = sum(len(t) for s, t in transcript_lines if s == "Ты")
            if candidate_chars < MIN_REPORT_CANDIDATE_CHARS:
                set_status("транскрипт пока слишком короткий для честного отчёта")
                return
            ...
            report_prompt = (
                ...
                "ВАЖНО: используй ТОЛЬКО факты и цитаты, которые реально есть в транскрипте ниже — "
                "если материала мало, так и напиши прямо ('мало данных, чтобы оценить X'), НЕ "
                "придумывай реплики, проекты, цифры или ситуации, которых там нет. "
